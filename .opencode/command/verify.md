...
| 3 | Bun compile smoke | Syntax + import-resolution errors in `plugin/*.ts` + `tool/*.ts` (NOT semantic typecheck) |
| 4 | `git diff --check` | Whitespace errors / conflict markers |

Exit 0 = all pass. Exit 1 = one or more failed — read the per-check FAIL lines.
