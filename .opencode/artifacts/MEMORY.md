...
### [2026-07-22] Repo-index — bounded on-demand locator experiment (negative result)

- **Context:** After skill-mine shipped, tried a bounded on-demand `/repo-index <scope>` command under the read-only `explore` agent — a ~40-line / ~1,500-token, `path:line`-citing locator map generated per call (entrypoints → direct modules/callers → recommended reads → omissions). Explicitly NOT a tracked file, NOT injected, NOT a verify check. The arXiv basis (AOCI 2605.02421, role-aware summaries 2607.11046, What-Context 2607.09691) framed summaries as *routing aids*, not source replacements — the hypothesis was a bounded locator would cut the localization turns the existing `explore`/`zoom-out` + auto-injected `AGENTS.md` already spend.
- **Decision:** **DELETE — did not ship.** A proof gate ran 3 tasks (this template's skill-mine promote flow + 2 consumers: personal-website/Astro Content Collections, portfolio-mastrabot/Mastra runtime) as a clean A/B — two fresh `explore` subagents per scope, baseline (bare "locate X") vs the `/repo-index` command body, both reporting auditable tool-call counts + traces + wrong-file starts.
- **Evidence:** `/repo-index` **increased** total localization turns 57 vs 35 (+22) and recovered **zero** wrong-file starts (1 WFS total across 6 runs). Per task: template +7, personal-website +17, portfolio-mastrabot −2. Root cause: the hard budget was on **output lines**, not **tool calls** — the 4-section completeness contract (entrypoints/callers/reads/omissions) incentivized **broader** search, not narrower. The one consumer wrong-file start (`src/index.ts` before `src/mastra/index.ts`) recurred in **both** legs, so the locator structure did not prevent it. 4 of 8 stop conditions held (#1 AGENTS.md+one targeted search already locates within the explore ≤3-calls/symbol budget; #2 no repeated WFS observed; #5 no net consumer benefit; #6 agents repeated broad searches, no net reduction) — any one of #1/#5/#6 alone warrants deletion.
- **Rationale:** A bounded-output locator duplicated what `explore`/`zoom-out` + the auto-injected `AGENTS.md` already do, just more expensively. Richer maps ≠ the success criterion (turn reduction). The conflict (command author ran the gate) is mitigated by the stop conditions pre-committing the decision to unambiguous data. An honest negative result is the valuable deliverable: it forecloses re-litigating a committed/injected repo map (the review audit's drift concern — README already drifted from the manifest — confirms a tracked snapshot would go stale).
- **Consequences:** No `/repo-index` command ships; `AGENTS.md` and `README.md` command tables stay as they were (no `/repo-index` row). `.opencode/artifacts/repo-index/{spec,prd.json,progress}.md` remain as the experiment record. `.opencode/artifacts/.active` removed (bead closed). Lesson for future cartography experiments: bound the **tool-call budget**, not just output lines, or a completeness contract drives over-search; and don't duplicate an auto-injected map that already fits the explore budget. Full data + per-task traces in `progress.md`.
